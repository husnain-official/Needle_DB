...
Parse_result insert_parsing(Vector &v, const std::string &command)
{
    try
    {
        ...
        // Embeddings loop
        v.data.resize(schema::DIMENSIONS);
...
Parse_result query_parsing(Vector &v, size_t &top_k, const std::string &command)
{
    try
    {
        ...
        size_t meta_count = 0;
        while (meta_count < schema::META_DATA_KP_PAIRS && next_space_index != std::string::npos)
        {
            ...
            std::strncpy(v.metadata[meta_count].key, key.c_str(), schema::META_DATA_LENGTH);
            std::strncpy(v.metadata[meta_count].value, val.c_str(), schema::META_DATA_LENGTH);
